...
const GUIDE_CONTENT = `# Sage User Guide
...
Access settings from the navigation bar or navigate to \`/settings\`.
...
export default function GuidePage() {
  ...
  const html = useMemo(() => {
    const raw = marked.parse(GUIDE_CONTENT) as string;
  ...
  return (
    <div className="flex-1 bg-background px-4 py-8">
      <div className="max-w-4xl mx-auto">
        <div
          className={`prose prose-sm max-w-none ${isDark ? 'prose-invert' : ''} [&_table]:w-full [&_table]:text-sm [&_th]:text-left [&_th]:py-2 [&_th]:px-3 [&_td]:py-2 [&_td]:px-3 [&_th]:border-b [&_th]:border-border [&_td]:border-b [&_td]:border-border [&_code]:text-primary [&_code]:bg-surface [&_code]:px-1.5 [&_code]:py-0.5 [&_code]:rounded [&_pre_code]:bg-transparent [&_pre_code]:p-0 [&_pre]:bg-surface [&_pre]:rounded-lg [&_pre]:p-4 [&_a]:text-primary [&_a]:no-underline hover:[&_a]:underline`}
